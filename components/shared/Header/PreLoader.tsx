...
export const PreLoader = () => {
  ...
  return (
    <Link
      ...
      <div
        className={cn(
          'fixed left-0 right-0 top-0 bottom-0 opacity-100 h-screen bg-black transition-all duration-500 flex items-center justify-center',
      ...
      <div className={cn('loading rounded-br-3xl transition-all', render ? 'move' : 'md:-ml-5')}>
        <Logo
          className={cn('transition-all duration-500', render ? 'h-7' : 'h-[60px] max-md:h-[40px]')}
        />
      </div>
    </Link>
  );
